...
@section('title', 'Incident Reports Management')
...
<div class="container-fluid px-0">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
        <div>
            <h1 class="h3 fw-bold text-dark mb-1">Incident Reports Management</h1>
            <p class="text-muted small mb-0">Monitor and handle all community incident and blotter reports.</p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 p-4" style="min-height: 750px; padding-bottom: 120px !important;">
        ...
        <form method="GET" action="{{ route('admin.incidents.index') }}" class="row g-3 mb-4 align-items-center justify-content-between">
            <div class="col-md-5">
                <div class="input-group">
                    <span class="input-group-text bg-light border-end-0 text-muted ps-3"><i class="bi bi-search"></i></span>
                    <input type="text" name="search" class="form-control bg-light border-start-0 ps-0 shadow-none" placeholder="Search resident, incident title, or details..." value="{{ request('search') }}">
                </div>
            </div>
        ...
        <div class="table-responsive overflow-visible">
            <table class="table table-hover align-middle mb-0">
                <thead class="table-light text-uppercase fs-7 text-muted" style="font-size: 0.75rem; letter-spacing: 0.5px;">
                    <tr>
                        <th class="py-3 ps-3 rounded-start-3">Incident ID</th>
                ...
                <tbody>
                    ...
                    <tr class="border-bottom border-light">
                        <td class="ps-3 font-monospace text-primary fw-bold">#{{ $incident->id }}</td>
                        <td>
                            <div class="fw-semibold text-dark">
                                @if($incident->user)
                                    {{ trim(($incident->user->first_name ?? '') . ' ' . ($incident->user->middle_name ?? '') . ' ' . ($incident->user->last_name ?? '')) ?: $incident->user->name }}
                                @else
                        ...
                        <td>
                            <div class="fw-medium text-dark">{{ $incident->incident_type ?? $incident->title ?? 'N/A' }}</div>
                        </td>
                        <td>
                            <span class="text-muted small" title="{{ $incident->description }}">
                        ...
                        <td>
                            <div class="d-flex align-items-center gap-1 text-secondary small fw-medium">
                                ...
                                <span>{{ $incident->created_at ? $incident->created_at->format('M d, Y') : 'N/A' }}</span>
                            </div>
                        </td>
                        <td>
                            @php
                                ...
                                if ($status == 'pending') {
                                    ...
                                    $dotColor = 'bg-dark';
                                } elseif ($status == 'investigating' || $status == 'in_progress') {
                        ...
                        <td class="text-end pe-3">
                            <div class="dropdown">
                                ...
                                <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-3 py-2">
                                    <li>
                                        <a class="dropdown-item py-2 px-3 text-dark small fw-medium" href="{{ route('admin.incidents.show', $incident->id) }}">
                    ...
                    <tr>
                        <td colspan="7" class="text-center py-5 text-muted">
                            <div class="py-4">
                                <i class="bi bi-shield-exclamation fs-1 text-muted opacity-50 mb-3 d-block"></i>
                                <h6 class="fw-bold text-dark">No incident reports found</h6>
                                <p class="small text-muted mb-0">There are currently no incident reports matching your filter.</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
